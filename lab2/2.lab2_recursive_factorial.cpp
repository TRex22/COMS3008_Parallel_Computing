...
usage: ./example2.out n averageNumber incrementSize noIncrements writeFile? noThreads
...
void initFileOuts()
{
	char header1[50] = "Recursive Factorial, Time, Factorial";
...
int main (int argc, char* argv[])
{
	...
	for (int i = 0; i < experimentNumber; i++)
	{
		for (int j = 0; j < averageNumber; j++)
		{
			...
			unsigned int factorial = calcRecursiveFactorial(n);
